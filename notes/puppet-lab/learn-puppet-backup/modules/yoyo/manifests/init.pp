class yoyo (
  ...
  # $facts is a hash containing system facts.
  # ['networking']['hostname'] gets the machine hostname.
  #
  # Example result:
  #   'host-01'
  ...
  if $local_imports != undef {
    ...
    # Convert a simple array like:
    #   ['coffee', 'tea']
    #
    # into a hash like:
    # {
    #   'coffee' => '/tmp/yoyo_xyz/inbox/host-01/coffee',
    #   'tea'    => '/tmp/yoyo_xyz/inbox/host-01/tea',
    # }
    #
    # "reduce({})" means:
    # start with an empty hash {} and build it up item by item.
    #
    # |$memo, $type| are the block variables:
    # - $memo = accumulated result so far
    # - $type = current array item, e.g. "coffee"
    $local_import_paths = $local_imports.reduce({}) |$memo, $type| {
